...
import static io.github.mayubao.kuaichuan.AppContext.getAppContext;
...
//                btn_selected.setTextColor(getResources().getColor(R.color.darker_gray));
                if (!getAppContext().isFileInfoMapExist()) {//不存在选中的文件
                ...
                /**
                 * 判断是由何种方式发送文件，蓝牙为了便于处理，限制一次只能发送一个文件
                 */
                if (getIntent().getStringExtra("type") != null && getIntent().getStringExtra("type").equals("blue")) {
                    Map<String, FileInfo> fileInfoMap = AppContext.getAppContext().getFileInfoMap();
                    if (fileInfoMap.size() > 1) {
                        ToastUtils.show(getContext(), "只能选择1个文件进行传输");
                        return;
                    }
                    //这里注释的是调用系统蓝牙发送文件代码
//                    File file = new File(fileInfoMap.entrySet().iterator().next().getValue().getFilePath());
//
...
//                    startActivity(Intent.createChooser(sharingIntent, "Share"));
                    Intent intent = new Intent(ChooseFileActivity.this, BlueSendActivity.class);
    ...
    public View getSelectedView() {
        ...
        if (getAppContext().getFileInfoMap() != null && getAppContext().getFileInfoMap().size() > 0) {
            setSelectedViewStyle(true);
            int size = getAppContext().getFileInfoMap().size();
