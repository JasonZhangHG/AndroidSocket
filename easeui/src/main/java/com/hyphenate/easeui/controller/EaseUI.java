...
public final class EaseUI {
    public static String headTokens = "";
    public static String nameTokens = "";

    public static void initToken(String nameToken, String headToken) {
        nameTokens = nameToken;
        headTokens = headToken;
    }
    ...
    private void registerMessageListener() {
        EMClient.getInstance().chatManager().addMessageListener(new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                /**
                 * 在application中开启监听了，在这里监听数据，把用户头像，用户名称等数据添加到数据库，会话列表自己编写（查询数据库）
                 * 聊天内容界面，通过查询username进行获取用户头像数据，判断如果是自己的username，直接显示自己的用户数据
                 */
                try {
                    Log.e("easeUI", "接受：" + messages.get(0).getStringAttribute("nickName"));
                    Log.e("easeUI", "接受：" + messages.get(0).getStringAttribute("headUrl"));
